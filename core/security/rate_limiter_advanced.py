...
class AdvancedRateLimiter:
    ...
    def configure(
        ...
        logger.debug(
            f"Configured rate limiter for {key}: {limit} per {period}s using {strategy.value}"
        )
    ...
    async def _check_local(
        ...
        if config.strategy == RateLimitStrategy.FIXED_WINDOW:
            if strategy_key not in self._strategies:
                self._strategies[strategy_key] = SlidingWindowCounter(config.limit, config.period)
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if args and hasattr(args[0], "headers"):
                args[0].headers["X-RateLimit-Limit"] = str(
                    limit or advanced_rate_limiter._get_config(rate_key).limit
                )
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            allowed, _remaining, retry_after = loop.run_until_complete(
