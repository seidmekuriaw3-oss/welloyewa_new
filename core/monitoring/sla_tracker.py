...
class SLATracker:
    ...
    def set_target(self, metric: SLAMetric, target: SLATarget) -> None:
        ...
        logger.info(
            f"SLA target set for {metric.value}: {target.target_value} ({target.comparison})"
        )
    ...
    def get_current_status(self, metric: SLAMetric) -> SLATarget | None:
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        breached = [name for name, s in statuses.items() if s["status"] == "breached"]
