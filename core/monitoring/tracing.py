...
class Tracer:
    ...
    def finish_trace(self, trace_id: str | None = None) -> None:
        ...
        if trace_id and trace_id in self._traces:
            ...
            logger.info(
                f"Finished trace: {trace.name} (Duration: {trace.duration_ms:.2f}ms, Spans: {len(trace.spans)})"
            )
    ...
    def get_trace_by_id(self, trace_id: str) -> Trace | None:
...
def trace_transaction(name: str, attributes: dict[str, Any] | None = None):
