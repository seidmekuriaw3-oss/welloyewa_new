...
class RealTimeAnalytics:
    ...
    async def track_add_to_cart(
        self, user_id: int, product_id: int, quantity: int = 1, **kwargs
    ) -> None:
    ...
    async def track_purchase(
        self, user_id: int, product_id: int, order_id: int, price: float, **kwargs
    ) -> None:
        """Convenience method to track purchase."""
        action = UserAction(
            user_id=user_id,
            action_type="purchase",
            data={"product_id": product_id, "order_id": order_id, "price": price, **kwargs},
        )
        await self.track_action(action)
    ...
    async def get_hot_products(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        # Sort by view count
        sorted_products = sorted(recent_views.items(), key=lambda x: x[1], reverse=True)[:limit]

        return [{"product_id": pid, "view_count": count} for pid, count in sorted_products]
    ...
    async def get_trending_products(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        sorted_products = sorted(recent_purchases.items(), key=lambda x: x[1], reverse=True)[:limit]

        return [{"product_id": pid, "purchase_count": count} for pid, count in sorted_products]
    ...
    async def get_popular_search_queries(
        ...
        sorted_queries = sorted(search_counts.items(), key=lambda x: x[1], reverse=True)[:limit]

        return [{"query": query, "count": count} for query, count in sorted_queries]
    ...
    async def clear_old_data(self, days: int = 7) -> None:
        ...
        self._action_buffer = deque(
            [a for a in self._action_buffer if a.timestamp >= cutoff], maxlen=10000
        )
