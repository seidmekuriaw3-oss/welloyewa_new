...
class ABTestingFramework:
    ...
    def assign_variant(self, experiment_id: str, user_id: int) -> ExperimentVariant | None:
        """
        ...
        for variant, allocation in experiment.traffic_allocation.items():
            ...
            if hash_percent < cumulative:
                ...
                logger.debug(
                    f"Assigned user {user_id} to {variant.value} for experiment {experiment_id}"
                )
                return variant
    ...
    async def track_conversion(
        ...
        logger.debug(
            f"Tracked conversion for experiment {experiment_id}, user {user_id}, variant {variant.value}"
        )
    ...
    async def get_experiment_results(self, experiment_id: str) -> list[ExperimentResult]:
        ...
        experiment_conversions = [c for c in self._conversions if c.experiment_id == experiment_id]
        ...
        for variant, data in variant_data.items():
            ...
            if control_data and variant != ExperimentVariant.CONTROL:
                control_rate = (
                    (control_data["conversions"] / len(control_data["users"]))
                    if len(control_data["users"]) > 0
                    else 0
                )
                if control_rate > 0:
    ...
    async def get_winning_variant(self, experiment_id: str) -> ExperimentVariant | None:
        """
