...
class RecommendationEngine:
    ...
    async def update_product_similarity(
        self, product_id: int, similar_products: list[tuple[int, float]]
    ) -> None:
        """
        ...
        logger.debug(
            f"Updated similarity for product {product_id}: {len(similar_products)} similar products"
        )

    async def update_frequently_bought_together(
        self, product_id: int, related: list[tuple[int, int]]
    ) -> None:
        """
        ...
        logger.debug(
            f"Updated frequently bought for product {product_id}: {len(related)} related products"
        )
    ...
    async def _update_frequently_bought_together(
        self, product_id: int, user_id: int, order_id: int
    ) -> None:
    ...
    async def get_hybrid_recommendations(
        ...
        if user_id:
            ...
            if len(similar) < limit:
                ...
                together = await self.get_frequently_bought_together(
                    product_id, limit - len(similar)
                )
    ...
    async def update_cache(self) -> None:
        ...
        try:
            ...
            await redis.setex("rec:popular", self._cache_ttl, str([r.product_id for r in popular]))
