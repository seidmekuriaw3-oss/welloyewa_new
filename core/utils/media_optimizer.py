...
from core.constants import ALLOWED_IMAGE_TYPES, MAX_IMAGE_SIZE_BYTES
...
class MediaOptimizer:
    ...
    @classmethod
    def detect_format(cls, image_data: bytes) -> str | None:
    ...
    @staticmethod
    def validate_image(image_data: bytes) -> tuple[bool, str | None]:
        """
        Validate image file.
        ...
        try:
            with Image.open(io.BytesIO(image_data)) as img:
                ...
                if img.format.lower() not in [t.split("/")[-1] for t in ALLOWED_IMAGE_TYPES]:
                    return (
                        False,
                        f"Unsupported image format. Allowed: {', '.join(ALLOWED_IMAGE_TYPES)}",
                    )
...
def generate_thumbnail(image_data: bytes, size: tuple[int, int] | None = None) -> bytes:
