...
class EthiopianCalendar:
    ...
    def get_month_days(cls, ethiopian_year: int, month: int) -> int:
        ...
        if month == 13:
            return (
                cls.LEAP_MONTH_DAYS[12]
                if cls.is_ethiopian_leap_year(ethiopian_year)
                else cls.MONTH_DAYS[12]
            )
    ...
    def days_since_reference(
        cls, ethiopian_year: int, ethiopian_month: int, ethiopian_day: int
    ) -> int:
    ...
    def from_gregorian(cls, gregorian_date: date) -> EthiopianDate:
        ...
        for month in range(1, 14):
            ...
            if days_remaining < month_days:
                return EthiopianDate(year=ethiopian_year, month=month, day=days_remaining + 1)
    ...
    def to_gregorian(cls, ethiopian_year: int, ethiopian_month: int, ethiopian_day: int) -> date:
        ...
        if ethiopian_day < 1 or ethiopian_day > max_day:
            raise ValueError(
                f"Invalid day: {ethiopian_day} for month {ethiopian_month}. Max: {max_day}"
            )
...
def convert_to_ethiopian(gregorian_date: date | datetime | str) -> EthiopianDate:
...
def format_ethiopian_date(
    date_obj: EthiopianDate | date | datetime, include_weekday: bool = False
) -> str:
