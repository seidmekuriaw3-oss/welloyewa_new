...
class TestInventoryService:
    ...
    async def test_reserve_stock(self):
        ...
        with patch("apps.inventory.services.InventoryRepository", return_value=mock_repo):
            with patch(
                "apps.inventory.services.StockReservationRepository",
                return_value=mock_reservation_repo,
            ):
                service = InventoryService(mock_db)
    ...
    async def test_release_reservation(self):
        ...
        with patch(
            "apps.inventory.services.StockReservationRepository", return_value=mock_reservation_repo
        ):
            with patch(
                "apps.inventory.services.InventoryRepository", return_value=mock_inventory_repo
            ):
                service = InventoryService(mock_db)
