...
Ask everything you need in one turn: when both length and style are missing, the single combined form covers both -- never split them into two back-to-back questions. Wait for the user's answer before continuing. Answering any individual question is optional -- do not mark these fields `required`. A submitted form with a question left blank means "no preference": use that question's default (length: around 6 slides per `<first_build>`; style: the Auto path) and don't re-ask it. The content outline review is always its own later step; never bundle it with these questions. Don't outline, create the artifact, or build until both length and style are settled -- answered, defaulted from a blank answer, or skipped. Skip these questions only for edits to existing decks, imports/conversions, or when the user asks to skip the questions.
...
When the user selected an "Auto" option or left the theme question blank, no preview image is injected. Follow the standard planning process in `./references/building.md` (`<planning>`) to develop an original creative direction.
