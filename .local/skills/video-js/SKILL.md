...
<delegation_required>
**Always delegate animated video work to the design subagent.** Do not do the work yourself. Complete the ratio step and start a design subagent and let it handle the entire video from start to finish. The design subagent has capabilities you don't have (image generation, video generation, web search) that are critical for producing high-quality videos. Animated video is a design task -- it belongs with the design subagent for both initial builds and iterations.

**What to do:**

**Ratio step (before any other work).** Read `.local/skills/video-js/references/resolution.md`. For a NEW video, complete its required ratio question before research, artifact creation, planning, template selection, or delegation. If you ask the question, end your turn and wait for the user's answer. Skip the question only when that reference permits it. Before delegation, resolve the selected ratio to one exact `selectedVideoAspectRatio` value.
1. Call `subagent()` with `config: { $kind: "design" }`, passing:
   - The user's request as the task, prefixed with: "You are the subagent executing this task, see the skill: .local/skills/video-js/SKILL.md\n\n"
   - **For every NEW video**, append: "Use the <selectedVideoAspectRatio> aspect ratio. This ratio is authoritative. Persist it before writing the composition." Replace the placeholder with the exact ratio from the user's request, unambiguous platform or orientation, or `AskQuestion` answer. Do not delegate a new video without this instruction.
   - `.local/skills/video-js/SKILL.md`, `.local/skills/video-js/references/resolution.md`, and `.local/skills/video-js/references/finalize-playback.md` as `relevantFiles` -- this auto-opens the skill for the subagent so it can read it directly
...
subagent({
    name: "video-design",
    task: `You are the subagent executing this task, see the skill: .local/skills/video-js/SKILL.md\n\n${userRequest}\n\nUse the ${selectedVideoAspectRatio} aspect ratio. This ratio is authoritative. Persist it before writing the composition.`,
    config: {
        $kind: "design",
        relevantFiles: [".local/skills/video-js/SKILL.md", ".local/skills/video-js/references/resolution.md", ".local/skills/video-js/references/finalize-playback.md"],
    },
});
```
...
<resolution>
Before creative work, read `references/resolution.md`. Follow its aspect-ratio instructions exactly.
</resolution>
