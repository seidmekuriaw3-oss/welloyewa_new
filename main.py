...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Database
    try:
        from infrastructure.database.session import close_db, init_db
    ...
    if (
        ...
        and not os.environ.get("REPLIT_DEPLOYMENT", "").strip() == "1"
    ):
        try:
...
async def detailed_health_check():
    ...
    from infrastructure.api.v1.endpoints.health import detailed_health_check as check
...
async def health_liveness_probe():
    ...
    return {"alive": True, "service": settings.PROJECT_NAME}
