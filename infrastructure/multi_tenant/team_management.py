...
class TeamManager:
    ...
    async def get_member(self, team_id: str, user_id: int) -> TeamMember | None:
        """Get a team member."""
        team = await self.get_team(team_id)
        if not team:
            return None
...
async def add_team_member(
    db, team_id: str, user_id: int, email: str, name: str, role: TeamRole, added_by: int
) -> TeamMember | None:
    """Add a member to a team."""
    manager = TeamManager(db)
...
async def update_member_role(
    db, team_id: str, user_id: int, new_role: TeamRole, updated_by: int
) -> bool:
    """Update a member's role."""
    manager = TeamManager(db)
