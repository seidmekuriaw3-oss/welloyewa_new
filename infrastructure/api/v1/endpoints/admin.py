...
async def admin_restore_backup(
    ...
    if success:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Restore failed"
        )
...
async def admin_update_user_status(
    user_id: int,
    status: str = Query(..., pattern="^(active|inactive|suspended|banned)$"),
    current_user: dict = Depends(get_current_admin),
    db: AsyncSession = Depends(get_db_session),
) -> MessageResponse:
    """
...
async def admin_get_vendors(
    ...
    if is_approved is not None:
        if is_approved:
            vendors = await vendor_service.vendor_repo.get_approved_vendors(
