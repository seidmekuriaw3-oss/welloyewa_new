...
class DataResidencyManager:
    ...
    def classify_data(
        self, data_type: str, contains_pii: bool = False, is_financial: bool = False
    ) -> DataClassification:
        """
    ...
    async def ensure_data_residency(
        ...
        data_type: str,
        classification: DataClassification,
        target_region: DataRegion | None = None,
    ) -> bool:
        """
        ...
        logger.info(
            f"Data {data_id} stored in {region.value} (classification: {classification.value})"
        )
        return True

    async def get_data_location(self, data_id: str) -> DataLocation | None:
    ...
    async def migrate_data(
        ...
        if not self.is_region_allowed(location.classification, target_region):
            logger.warning(
                f"Region {target_region.value} not allowed for classification {location.classification.value}"
            )
            return False
    ...
    async def get_compliance_report(self) -> dict[str, Any]:
        ...
        for location in self._data_locations.values():
            ...
            classifications_count[location.classification.value] = (
                classifications_count.get(location.classification.value, 0) + 1
            )
...
async def get_data_location(data_id: str) -> DataLocation | None:
    """Get data location."""
...
class DataResidencyCompliance:
    ...
    _instance: DataResidencyManager | None = None
