...
class LegalInvoiceGenerator:
    ...
    def generate_invoice_number(self) -> str:
        ...
        invoice_num = (
            f"{self._invoice_prefix}{datetime.utcnow().strftime('%Y%m')}{self._invoice_counter:06d}"
        )
        self._invoice_counter += 1
        return invoice_num
    ...
    async def validate_invoice_for_tax(self, invoice: LegalInvoice) -> dict[str, Any]:
        ...
        for field_name in required_fields:
            if not getattr(invoice, field_name, None):
        ...
        if invoice.tax_result:
            expected_total = invoice.subtotal + invoice.shipping_fee - invoice.discount

            if abs(float(expected_total) - float(invoice.total_amount)) > 0.01:
                errors.append(
                    f"Tax calculation mismatch: expected {expected_total}, got {invoice.total_amount}"
                )
