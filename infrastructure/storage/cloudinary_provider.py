...
class CloudinaryProvider(StorageProvider):
    ...
    async def upload(
        ...
        try:
            ...
            raise UploadError(f"Failed to upload to Cloudinary: {e}") from e
    ...
    async def download(self, public_id: str) -> bytes:
        ...
        try:
            ...
            raise FileNotFoundError(f"File not found: {public_id}") from exc
        except Exception as e:
            logger.error(f"Cloudinary download failed: {e}")
            raise DownloadError(f"Failed to download from Cloudinary: {e}") from e
    ...
    async def get_url(
        ...
        if transformations:
            url = cloudinary.utils.cloudinary_url(
                public_id, resource_type="auto", transformation=transformations
            )[0]
        else:
            url = cloudinary.utils.cloudinary_url(public_id, resource_type="auto")[0]
    ...
    async def get_metadata(self, public_id: str) -> StorageFile | None:
        """
