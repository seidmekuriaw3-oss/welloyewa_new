...
class S3StorageProvider(StorageProvider):
    ...
    async def _get_client(self):
        ...
        if not self._initialized:
            try:
                ...
                logger.info(
                    f"S3 storage initialized: bucket={self.bucket_name}, region={self.region}"
                )
    ...
    async def upload(
        ...
        try:
            ...
            # Upload file
            await client.put_object(Bucket=self.bucket_name, Key=key, Body=file_data, **extra_args)
    ...
    async def download(self, key: str) -> bytes:
        ...
        try:
            ...
            raise DownloadError(f"Failed to download from S3: {e}") from e
