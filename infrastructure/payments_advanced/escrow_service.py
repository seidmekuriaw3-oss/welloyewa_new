...
class EscrowService:
    ...
    async def resolve_dispute(
        ...
        if resolve_in_favor_of == "buyer":
            success = await self.refund_buyer(
                escrow_id, "Dispute resolved in favor of buyer", admin_id
            )
        ...
        if success:
            ...
            await self._update_escrow(escrow)
            logger.info(
                f"Dispute resolved for escrow {escrow_id} in favor of {resolve_in_favor_of}"
            )
            return True
    ...
    async def _get_escrow(self, escrow_id: str) -> EscrowTransaction | None:
...
async def create_escrow(
    ...
    return await service.create_escrow(
        order_id, order_number, buyer_id, seller_id, amount, holding_days
    )
