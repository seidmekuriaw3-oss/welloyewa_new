...
class SatisfactionRating(StrEnum):
...
class SurveyResponse:
    ...
    rating: SatisfactionRating | None = None
...
class CustomerSatisfaction:
    ...
    def add_response(self, response: SurveyResponse) -> None:
        ...
        logger.info(
            f"Added survey response from user {response.user_id}: rating {response.rating_value}/5"
        )
    ...
    def get_csat_score(
        ...
        responses = [r for r in self._responses if r.created_at >= cutoff and r.rating_value > 0]
    ...
    def get_satisfaction_stats(self, days: int = 30) -> dict[str, Any]:
        ...
        total_rating = 0
        for r in responses:
            rating_distribution[str(r.rating_value)] = (
                rating_distribution.get(str(r.rating_value), 0) + 1
            )
...
async def collect_feedback(response_id: str, rating: int, feedback: str | None = None) -> bool:
    """Collect feedback from user."""
