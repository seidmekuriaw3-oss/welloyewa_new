...
class CannedResponseManager:
    ...
    def get_response(self, response_id: str) -> CannedResponse | None:
    ...
    def get_responses_by_category(self, category: ResponseCategory) -> list[CannedResponse]:
        ...
        return [r for r in self._responses.values() if r.category == category and r.is_active]
    ...
    def search_responses(self, keyword: str) -> list[CannedResponse]:
        ...
        for response in self._responses.values():
            ...
            if (
                keyword_lower in response.title.lower()
                or keyword_lower in response.content.lower()
                or (response.content_am and keyword_lower in response.content_am.lower())
            ):
                matches.append(response)
...
async def get_canned_response(response_id: str) -> CannedResponse | None:
    """Get a canned response by ID."""
