...
class SLAMonitor:
    ...
    def record_first_response(self, ticket_id: int, response_time: datetime) -> None:
        ...
        if response_minutes > tracker.first_response_target:
            violation = SLAViolation(
                ...
                severity=(
                    "critical"
                    if response_minutes > tracker.first_response_target * 2
                    else "warning"
                ),
            )
            self._violations.append(violation)
            logger.warning(
                f"SLA violation: First response time for ticket {ticket_id} exceeded target"
            )
                    ...
                    risks.append(
                        {
                            ...
                            "target_minutes": tracker.first_response_target,
                            "risk_level": (
                                "high" if elapsed > tracker.first_response_target else "medium"
                            ),
                        }
                    )
    ...
    def get_sla_report(self) -> dict[str, Any]:
        ...
        first_response_violations = sum(
            1 for v in self._violations if v.metric == SLAMetric.FIRST_RESPONSE_TIME
        )
        resolution_violations = sum(
            1 for v in self._violations if v.metric == SLAMetric.RESOLUTION_TIME
        )
        ...
        return {
            ...
            "compliance_rate": round(
                (total_tickets - first_response_violations - resolution_violations)
                / max(total_tickets, 1)
                * 100,
                1,
            ),
