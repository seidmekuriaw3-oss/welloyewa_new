...
class InlineKeyboardBuilderWrapper:
    ...
    def add_button(
        self, text: str, callback_data: str, row: int | None = None
    ...
    def add_buttons(
        self, buttons: list[tuple], row: int | None = None
    ) -> "InlineKeyboardBuilderWrapper":
...
class ReplyKeyboardBuilderWrapper:
    ...
    def add_button(
        ...
        button = KeyboardButton(
            text, request_contact=request_contact, request_location=request_location
        )
    ...
    def add_buttons(
        self, buttons: list[str], row: int | None = None
    ) -> "ReplyKeyboardBuilderWrapper":
        """Add multiple buttons."""
        for text in buttons:
...
def build_product_grid(
    ...
    for product in products:
        button = InlineKeyboardButton(
            f"📦 {product['name'][:20]}", callback_data=f"prod_{product['id']}"
        )
        row.append(button)
...
def build_category_list(
    ...
    for category in categories:
        button = InlineKeyboardButton(
            f"📁 {category['name']}", callback_data=f"cat_{category['id']}"
        )
        row.append(button)
