...
def _get_wishlist(context: ContextTypes.DEFAULT_TYPE) -> list:
    ...
    wishlist = user_data.get(WISHLIST_KEY, []) if hasattr(user_data, "get") else []
    return wishlist if isinstance(wishlist, list) else []
...
async def wishlist_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not wishlist:
        ...
        if getattr(update, "message", None) is None and update.callback_query:
            await update.callback_query.message.edit_text(msg, parse_mode="Markdown")
            await update.callback_query.answer()
        else:
            await maybe_await(update.message.reply_text(msg, parse_mode="Markdown"))
        return
