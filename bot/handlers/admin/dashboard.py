...
from apps.analytics.services import SalesAnalyticsService
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Products ────────────────────────────────────────────────────
        ...
        context.user_data["admin_images_page"] = context.user_data.get("admin_images_page", 1) + 1
        from bot.handlers.admin.products_admin import list_products_for_images
    ...
    # ── Reports ─────────────────────────────────────────────────────
        ...
        await query.message.reply_text(f"⚠️ ያልታወቀ ትዕዛዝ: `{action}`", parse_mode="Markdown")
