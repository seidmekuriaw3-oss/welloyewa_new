...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    query = update.callback_query if getattr(update, "message", None) is None else None
    ...
    if query:
        await maybe_await(query.message.edit_text(
            message_text, parse_mode="Markdown", reply_markup=reply_markup
        ))
        await maybe_await(query.answer())
    else:
...
async def category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        pass
    await maybe_await(show_category_products(update, context, category_id, page=1))
