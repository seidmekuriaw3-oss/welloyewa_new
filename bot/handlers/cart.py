...
def _get_cart(context: ContextTypes.DEFAULT_TYPE) -> list:
    ...
    cart = user_data.get(CART_KEY, []) if hasattr(user_data, "get") else []
    ...
    if redis_client is not None:
        await maybe_await(
            redis_client.setex(
                f"cart:{user_id}",
                86400,
                json.dumps({"product_id": product_id, "quantity": 1}),
            )
        )
...
def get_redis_client():
    """Return the optional legacy Redis client, when configured by callers."""
    return None
