...
async def _vendor_for(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not vendor or not vendor.is_approved:
        await update.effective_message.reply_text("❌ የሻጭ መለያዎ ገና አልጸደቀም።", reply_markup=_back())
        return None
    return vendor
...
async def vendor_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "vendor_products":
        ...
        await query.message.edit_text(
            "+ *አዲስ ምርት*\n\nየምርቱን ስም ይላኩ።", parse_mode="Markdown", reply_markup=_back()
        )
        ...
        async for db in get_db_session():
            ...
            break
        await query.message.edit_text(
            "✅ ምርቱ ተሰርዟል።" if ok else "❌ ምርቱን ማጥፋት አልተቻለም።", reply_markup=_back()
        )
    elif action == "vendor_products_next":
        context.user_data["vendor_products_page"] = (
            context.user_data.get("vendor_products_page", 1) + 1
        )
        await list_vendor_products(update, context)
    elif action == "vendor_products_prev":
        context.user_data["vendor_products_page"] = max(
            1, context.user_data.get("vendor_products_page", 1) - 1
        )
        await list_vendor_products(update, context)
        ...
        context.user_data["vendor_orders_page"] = max(
            1, context.user_data.get("vendor_orders_page", 1) - 1
        )
        await list_vendor_orders(update, context)
...
async def list_vendor_products(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async for db in get_db_session():
        products = await ProductService(db).get_vendor_products(
            vendor.id, limit=8, offset=(page - 1) * 8
        )
        break
    text = f"📦 *ምርቶቼ* — ገጽ {page}\n\n"
    keyboard = []
    for product in products:
        text += (
            f"• *{product.name}* — {format_etb(product.price)} | stock: {product.stock_quantity}\n"
        )
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"✏️ {product.name[:25]}", callback_data=f"vendor_edit_{product.id}"
                )
            ]
        )
    if not products:
...
async def list_vendor_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async for db in get_db_session():
        orders, total = await OrderService(db).get_vendor_orders(
            vendor.id, limit=6, offset=(page - 1) * 6
        )
        break
    ...
    for order in orders:
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"🔍 #{order.order_number}", callback_data=f"vendor_order_{order.id}"
                )
            ]
        )
    if not orders:
...
async def show_vendor_order(
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton(
                "🔄 Processing", callback_data=f"vendor_status_{order_id}_processing"
            ),
        ],
        [
...
async def vendor_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if state == "name":
        ...
        sku = (
            f"{re.sub(r'[^A-Z0-9]', '', name.upper())[:8] or 'PROD'}-{uuid.uuid4().hex[:8].upper()}"
        )
        async for db in get_db_session():
        ...
        await update.effective_message.reply_text(
            f"✅ *{product.name}* ተፈጥሯል።", parse_mode="Markdown", reply_markup=_back()
        )
        ...
        try:
            data = (
                ProductUpdate(price=Decimal(value))
                if state == "edit_price"
                else ProductUpdate(stock_quantity=int(value))
            )
        ...
        if vendor:
            async for db in get_db_session():
                await VendorService(db).update_vendor(vendor.id, VendorUpdate(business_name=value))
                break
        context.user_data.pop("vendor_state", None)
