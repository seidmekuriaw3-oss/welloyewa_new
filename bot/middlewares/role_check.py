...
def vendor_only(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        ...
        if (
            user_role not in ["vendor", "admin", "super_admin"]
            and user_id not in settings.admin_ids_list
        ):
            await update.message.reply_text(
...
__all__ = ["RoleCheckMiddleware", "admin_only", "role_check_middleware", "vendor_only"]
