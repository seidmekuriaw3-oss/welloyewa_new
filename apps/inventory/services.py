...
from apps.inventory.repository import (
    InventoryMovementRepository,
    InventoryRepository,
    StockReservationRepository,
)
from apps.inventory.schemas import InventoryCreate, InventoryMovementCreate, InventoryUpdate
from core.events import PRODUCT_STOCK_LOW, PRODUCT_STOCK_OUT, emit_event
...
class InventoryService:
    ...
    async def get_inventory(self, product_id: int) -> Inventory | None:
    ...
    async def add_stock(
        ...
        logger.info(
            f"Added {quantity} units to product {product_id}. New stock: {inventory.quantity}"
        )
        return inventory
    ...
    async def remove_stock(
        ...
        available_quantity = getattr(inventory, "available_quantity", None)
        if isinstance(available_quantity, (int, float)) and available_quantity < quantity:
            raise InsufficientStockError(
                f"Product {product_id}", quantity, inventory.available_quantity
            )
        ...
        logger.info(
            f"Removed {quantity} units from product {product_id}. New stock: {inventory.quantity}"
        )
        return inventory
    ...
    async def reserve_stock(
        ...
        await self.inventory_repo.update(
            inventory.id, {"reserved_quantity": inventory.reserved_quantity}
        )

        logger.info(
            f"Reserved {quantity} units of product {product_id} for {reference_type} {reference_id}"
        )
        return reservation
    ...
    async def release_reservation(self, reservation_id: int) -> None:
        ...
        status = getattr(reservation, "status", "active")
        if isinstance(status, str) and status not in ["active", "expired"]:
            return
    ...
    async def _check_stock_alerts(self, inventory: Inventory) -> None:
        ...
        if inventory.is_critical_stock:
            ...
            logger.info(
                f"Product {inventory.product_id} has low stock: {inventory.available_quantity}"
            )
...
class StockMovementService:
    ...
    async def get_movement_summary(
        ...
        for movement in movements:
            is_outgoing = movement.movement_type in {"sale", "remove", "shipment"}
            if movement.quantity > 0 and not is_outgoing:
...
__all__ = ["InventoryService", "ReservationService", "StockMovementService"]
