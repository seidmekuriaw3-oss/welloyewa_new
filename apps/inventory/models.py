...
class InventoryMovement(BaseModel, TimestampMixin):
    ...
    inventory_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("inventories.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    )  # order_id, purchase_order_id, etc.
    reference_type: Mapped[str | None] = mapped_column(
        String(50), nullable=True
    )  # order, return, adjustment

    # Metadata
    reason: Mapped[str | None] = mapped_column(Text, nullable=True)
    performed_by: Mapped[int | None] = mapped_column(BigInteger, nullable=True)  # user_id
...
class StockReservation(BaseModel, TimestampMixin):
    ...
    # Reservation details
    quantity: Mapped[int] = mapped_column(Integer, nullable=False)
    reference_id: Mapped[int | None] = mapped_column(BigInteger, nullable=True)  # order_id, cart_id
