...
async def _scalars_all(result) -> list:
    """Read scalar rows from both SQLAlchemy results and async test doubles."""
...
class InventoryRepository(BaseRepository[Inventory]):
    ...
    async def get_inventory_value(self, vendor_id: int | None = None) -> float:
        """Calculate total inventory value (sum of quantity * product price)."""
        from apps.products.models import Product

        query = select(func.sum(Inventory.quantity * Product.price)).join(
    ...
    async def get_inventory_stats(self, vendor_id: int | None = None) -> dict[str, Any]:
...
class InventoryMovementRepository(BaseRepository[InventoryMovement]):
    ...
    async def get_by_inventory(
        ...
        return await _scalars_all(result), total
...
class StockReservationRepository(BaseRepository[StockReservation]):
    ...
    async def get_expired_reservations(self) -> list[StockReservation]:
        ...
        query = select(StockReservation).where(
            StockReservation.expires_at <= now, StockReservation.status == "active"
        )
        result = await self.db.execute(query)
        return await _scalars_all(result)
...
__all__ = ["InventoryMovementRepository", "InventoryRepository", "StockReservationRepository"]
