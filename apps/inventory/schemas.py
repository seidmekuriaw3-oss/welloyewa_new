...
class InventoryBase(BaseSchema):
    ...
    shelf_location: str | None = Field(None, max_length=50, description="Shelf location")
...
class InventoryUpdate(BaseSchema):
    ...
    critical_stock_threshold: int | None = Field(None, ge=0)
...
class InventoryMovementBase(BaseSchema):
    ...
    movement_type: str = Field(
        ..., description="Movement type (purchase, sale, return, adjustment, restock)"
    )
    ...
    reason: str | None = Field(None, description="Reason for movement")
    reference_id: int | None = Field(None, description="Reference ID (order_id, etc.)")
    reference_type: str | None = Field(None, description="Reference type (order, return, etc.)")
    performed_by: int | None = Field(None, description="User ID who performed action")
...
class StockReservationResponse(StockReservationBase, IdSchema, TimestampSchema):
    ...
    status: str = Field(
        ..., description="Reservation status (active, confirmed, expired, cancelled)"
    )
...
class BulkStockUpdateRequest(BaseSchema):
    ...
    updates: list[StockAdjustmentRequest] = Field(
        ..., min_length=1, description="List of stock updates"
    )
...
class StockCountRequest(BaseSchema):
    ...
    notes: str | None = Field(None, description="Count notes")
...
class InventoryReportResponse(BaseSchema):
    ...
    by_vendor: dict[str, Any] | None = Field(None, description="Breakdown by vendor")
