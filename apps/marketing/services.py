...
from apps.marketing.models import Campaign, Coupon, LoyaltyTransaction, Promotion
...
class CouponService:
    ...
    async def apply_coupon(
        ...
        return await self.coupon_manager.apply_coupon(code, user_id, order_id, order_amount)
...
class LoyaltyService:
    ...
    async def get_user_tier(self, user_id: int) -> str | None:
...
class CampaignService:
    ...
    async def track_click(self, campaign_id: int, user_id: int | None = None) -> None:
        ...
        for promotion in promotions:
            if promotion.promotion_type == "discount":
                ...
                if quantity >= (promotion.buy_quantity or 0):
                    free_items = (quantity // (promotion.buy_quantity or 1)) * (
                        promotion.get_quantity or 0
                    )
...
__all__ = ["CampaignService", "CouponService", "LoyaltyService", "PromotionService"]
