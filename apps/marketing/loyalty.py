...
class LoyaltyManager:
    ...
    async def redeem_points(
        ...
        if points > self.max_redeem_per_order:
            raise ValidationError(
                f"Maximum redemption per order is {self.max_redeem_per_order} points"
            )
    ...
    async def expire_old_points(self, days: int | None = None) -> int:
...
async def get_points_history(
    ...
    manager = LoyaltyManager(db)
    transactions, _total = await manager.get_points_history(user_id, limit)
