...
from apps.marketing.models import Campaign, Coupon, CouponUsage, LoyaltyTransaction, Promotion
...
class CouponRepository(BaseRepository[Coupon]):
    ...
    async def get_valid_coupons_for_user(
        ...
        conditions = [
            ...
            Coupon.valid_from <= now,
            Coupon.valid_to >= now,
            or_(Coupon.min_purchase_amount.is_(None), Coupon.min_purchase_amount <= order_amount),
    ...
    async def get_expired_coupons(self) -> list[Coupon]:
        """Get expired coupons."""
        now = datetime.utcnow()
        query = select(Coupon).where(Coupon.valid_to < now, Coupon.is_active)
        result = await self.db.execute(query)
...
class CouponUsageRepository(BaseRepository[CouponUsage]):
    ...
    async def get_usage_count(self, coupon_id: int, user_id: int | None = None) -> int:
...
class CampaignRepository(BaseRepository[Campaign]):
    ...
    async def increment_clicks(self, campaign_id: int) -> None:
        ...
        await self.db.execute(
            update(Campaign).where(Campaign.id == campaign_id).values(clicks=Campaign.clicks + 1)
        )
        await self.db.flush()
    ...
    async def increment_conversions(self, campaign_id: int, revenue: Decimal) -> None:
        ...
        await self.db.execute(
            update(Campaign)
            .where(Campaign.id == campaign_id)
            .values(conversions=Campaign.conversions + 1, revenue=Campaign.revenue + revenue)
        )
        await self.db.flush()
...
class PromotionRepository(BaseRepository[Promotion]):
    ...
    async def get_active_promotions(
        ...
        if product_id:
            query = query.where(
                or_(Promotion.product_ids.contains([product_id]), Promotion.product_ids.is_(None))
            )

        if category_id:
            query = query.where(
                or_(
                    Promotion.category_ids.contains([category_id]), Promotion.category_ids.is_(None)
                )
            )
