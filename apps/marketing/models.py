...
class Coupon(BaseModel, TimestampMixin):
    ...
    # Usage limits
    usage_limit: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Total usage limit
    per_user_limit: Mapped[int | None] = mapped_column(Integer, nullable=True)  # Per user limit
    used_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)

    # Restrictions
    applicable_categories: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    applicable_products: Mapped[list[int] | None] = mapped_column(JSON, nullable=True)
    excluded_categories: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    excluded_products: Mapped[list[int] | None] = mapped_column(JSON, nullable=True)
...
class CouponUsage(BaseModel, TimestampMixin):
    ...
    coupon_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("coupons.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class LoyaltyProgram(BaseModel, TimestampMixin):
    ...
    points_per_birr: Mapped[float] = mapped_column(
        Float, nullable=False, default=1.0
    )  # Points per ETB spent
    ...
    max_redeem_per_order: Mapped[int | None] = mapped_column(Integer, nullable=True)

    # Tiers
    tier_enabled: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    tier_config: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class LoyaltyTransaction(BaseModel, TimestampMixin):
    ...
    __table_args__ = (Index("idx_loyalty_user_date", "user_id", "created_at"),)
...
class Campaign(BaseModel, TimestampMixin):
    ...
    campaign_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # discount, flash_sale, bundle, referral
    ...
    # Target audience
    target_segments: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    target_cities: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)
    ...
    promotions: Mapped[list["Promotion"]] = relationship(
        "Promotion", back_populates="campaign", cascade="all, delete-orphan"
    )
...
class CampaignRule(BaseModel, TimestampMixin):
    ...
    rule_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # min_order, first_purchase, user_segment
...
class Promotion(BaseModel, TimestampMixin):
    ...
    campaign_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("campaigns.id", ondelete="SET NULL"), nullable=True, index=True
    )

    # Promotion type
    promotion_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # discount, buy_x_get_y, free_shipping

    # Target
    product_ids: Mapped[list[int] | None] = mapped_column(JSON, nullable=True)
    category_ids: Mapped[list[int] | None] = mapped_column(JSON, nullable=True)
    ...
    get_product_ids: Mapped[list[int] | None] = mapped_column(JSON, nullable=True)
    ...
    def __repr__(self) -> str:
        return (
            f"<Promotion(id={self.id}, type={self.promotion_type}, discount={self.discount_value})>"
        )
