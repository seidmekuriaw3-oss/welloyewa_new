...
class RefundManager:
    ...
    async def process_refund(
        ...
        try:
            ...
            raise PaymentError(f"Refund processing failed: {e}") from e
    ...
    async def get_refund_status(self, refund_id: str) -> dict[str, Any] | None:
        """Get refund status."""
        refund = self._refunds.get(refund_id)
        if not refund:
            return None
    ...
    async def get_order_refunds(self, order_id: int) -> list[dict[str, Any]]:
        ...
        order_refunds = [refund for refund in self._refunds.values() if refund.order_id == order_id]
...
async def get_refund_status(refund_id: str, db) -> dict[str, Any] | None:
