...
class OrderRepository(BaseRepository[Order]):
    ...
    async def get_by_id(self, id: int) -> Order | None:
        """Get order by ID and eager-load its relationships for API serialization."""
        query = (
            select(Order)
    ...
    async def get_by_order_number(self, order_number: str) -> Order | None:
        ...
        query = select(Order).options(selectinload(Order.items), selectinload(Order.history)).where(
    ...
    async def get_by_vendor(
        ...
        if status:
            conditions.append(or_(Order.status == status, OrderItem.vendor_status == status))
    ...
    async def get_daily_sales(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        query = (
            ...
            .where(Order.created_at >= cutoff, Order.payment_status == PaymentStatus.PAID.value)
