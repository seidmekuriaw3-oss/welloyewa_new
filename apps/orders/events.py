...
from core.events import Event, EventPriority, emit_event, event_bus
...
def register_order_handlers() -> None:
    ...
    event_bus.subscribe(
        OrderEventType.CONFIRMED.value, on_order_confirmed, priority=EventPriority.HIGH
    )
    ...
    event_bus.subscribe(
        OrderEventType.CANCELLED.value, on_order_cancelled, priority=EventPriority.HIGH
    )
    event_bus.subscribe(
        OrderEventType.PAYMENT_RECEIVED.value, on_payment_received, priority=EventPriority.CRITICAL
    )
