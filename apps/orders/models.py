...
class Order(BaseModel, TimestampMixin):
    ...
    vendor: Mapped[Optional["Vendor"]] = relationship(
        "Vendor", foreign_keys=[vendor_id], back_populates="orders"
    )
    ...
    def cancel(self, reason: str | None = None, user_id: int | None = None) -> None:
        """Cancel the order."""
    ...
    def refund(self, amount: Decimal, transaction_id: str | None = None) -> None:
...
class OrderHistory(BaseModel, TimestampMixin):
    ...
    def __repr__(self) -> str:
        return (
            f"<OrderHistory(order_id={self.order_id}, {self.previous_status} -> {self.new_status})>"
        )
...
class Shipment(BaseModel, TimestampMixin):
    ...
    # Tracking history
    tracking_history: Mapped[list[dict] | None] = mapped_column(JSON, nullable=True)

    # Relationships
    order: Mapped["Order"] = relationship("Order")

    def add_tracking_update(
        self, status: str, location: str | None = None, note: str | None = None
    ) -> None:
