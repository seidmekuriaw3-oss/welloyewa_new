...
class OrderService:
    ...
    async def update_order_status(
        self, order_id: int, data: OrderStatusUpdate, user_id: int
    ) -> Order:
        """
        Update order status.
        ...
        logger.info(
            f"Order {order.order_number} status changed: {previous_status} -> {data.status}"
        )
        return order

    async def update_payment_status(
        self, order_id: int, payment_status: str, transaction_id: str | None = None
    ) -> Order:
        ...
        if payment_status == PaymentStatus.PAID.value:
            ...
            await self._add_history(
                order_id, order.status, OrderStatus.CONFIRMED.value, None, "Payment received"
            )
...
__all__ = ["OrderItemService", "OrderService", "OrderTrackingService"]
