...
class SalesReportRequest(BaseSchema):
    ...
    group_by: str | None = Field("day", description="Group by (day, week, month)")
...
class SalesReportResponse(BaseSchema):
    ...
    daily_breakdown: list[DailySalesData] = Field(
        default_factory=list, description="Daily breakdown"
    )
    weekly_breakdown: list[WeeklySalesData] = Field(
        default_factory=list, description="Weekly breakdown"
    )
    monthly_breakdown: list[MonthlySalesData] = Field(
        default_factory=list, description="Monthly breakdown"
    )
    top_products: list[TopProductData] = Field(
        default_factory=list, description="Top selling products"
    )
    top_categories: list[dict[str, Any]] = Field(
        default_factory=list, description="Top selling categories"
    )
    revenue_by_payment: list[dict[str, Any]] = Field(
        default_factory=list, description="Revenue by payment method"
    )
...
class UserActivityReport(BaseSchema):
    ...
    user_growth: list[UserGrowthData] = Field(
        default_factory=list, description="User growth over time"
    )
    retention: UserRetentionData = Field(..., description="User retention metrics")
    user_segments: dict[str, int] = Field(
        default_factory=dict, description="User segmentation counts"
    )
    conversion_funnel: dict[str, float] = Field(
        default_factory=dict, description="Conversion funnel metrics"
    )
...
class ProductPerformanceReport(BaseSchema):
    ...
    products: list[ProductPerformanceData] = Field(
        default_factory=list, description="Product performance list"
    )
    inventory: InventoryAnalyticsData = Field(..., description="Inventory analytics")
    low_stock_products: list[dict[str, Any]] = Field(
