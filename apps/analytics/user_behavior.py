...
class UserBehaviorAnalyzer:
    ...
    async def get_user_segments(self) -> dict[str, int]:
        ...
        for user in users:
            ...
            orders = await self.db.execute(
                select(Order).where(
                    Order.user_id == user.id, Order.status == OrderStatus.DELIVERED.value
                )
            )
    ...
    async def get_user_purchase_patterns(
        ...
        orders = await self.db.execute(
            select(Order)
            .where(Order.user_id == user_id, Order.status == OrderStatus.DELIVERED.value)
        ...
        return {
            ...
            "preferred_categories": [
                {"category": cat, "items": count} for cat, count in top_categories
            ],
        }
    ...
    async def get_product_recommendations(
        ...
        user_orders = await self.db.execute(select(Order).where(Order.user_id == user_id))
    ...
    async def _get_user_category_preferences(
        ...
        # Get user's orders
        orders = await self.db.execute(select(Order).where(Order.user_id == user_id))
        orders = orders.scalars().all()
    ...
    async def _get_popular_products(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        popular = await self.db.execute(
            select(Product)
            .where(Product.status == "active", Product.stock_quantity > 0)
    ...
    async def get_similar_users(
        ...
        similar_users = await self.db.execute(
            ...
            .where(Product.category.in_(user_categories), Order.user_id != user_id)
