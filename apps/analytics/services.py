...
from apps.analytics.repository import SalesRepository, UserActivityRepository
...
class ProductAnalyticsService:
    ...
    async def get_product_performance(
        ...
        return {
            ...
            "conversion_rate": (
                (product.sales_count / product.views_count * 100) if product.views_count > 0 else 0
            ),
...
class DashboardService:
    ...
    async def get_dashboard_summary(
        ...
        # Top products
        top_products = await self.sales_service.get_top_products(
            limit=5, days=30, vendor_id=vendor_id
        )
...
class AnalyticsService:
    ...
    async def generate_sales_report(
        ...
        top_products = await self.sales.get_top_products(
            limit=10, days=(end_date - start_date).days, vendor_id=vendor_id
        )
