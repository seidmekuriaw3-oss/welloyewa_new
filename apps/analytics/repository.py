...
class SalesRepository:
    ...
    async def get_product_sales(
        ...
        query = select(
            func.sum(OrderItem.quantity).label("total_quantity"),
            func.sum(OrderItem.total_price).label("total_revenue"),
        ).where(OrderItem.product_id == product_id, OrderItem.created_at >= cutoff)
...
class UserActivityRepository:
    ...
    async def get_user_retention(
        ...
        returning_users = {r[0] for r in orders_result.all()}
    ...
    async def get_conversion_funnel(self) -> dict[str, float]:
        ...
        # Completed purchases
        purchases_query = select(func.count(Order.id)).where(
            Order.payment_status == PaymentStatus.PAID.value
        )
        ...
        return {
            ...
            "cart_to_checkout": (
                (checkouts / max(cart_additions, 1) * 100) if cart_additions > 0 else 0
            ),
