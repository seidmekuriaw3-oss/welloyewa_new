...
from apps.support.models import FAQ, Ticket, TicketMessage
from apps.support.repository import FAQRepository, TicketMessageRepository, TicketRepository
...
class TicketService:
    ...
    async def get_ticket(
        self, ticket_id: int, user_id: int | None = None, is_admin: bool = False
    ) -> Ticket:
    ...
    async def update_ticket(
        self, ticket_id: int, data: TicketUpdate, is_admin: bool = False
    ) -> Ticket:
        """Update a ticket."""
    ...
    async def add_message(
        self, ticket_id: int, user_id: int, data: TicketMessageCreate, is_staff: bool = False
    ) -> TicketMessage:
    ...
    async def assign_ticket(self, ticket_id: int, assigned_to: int, admin_id: int) -> Ticket:
        ...
        logger.info(
            f"Ticket {ticket.ticket_number} assigned to agent {assigned_to} by admin {admin_id}"
        )
        return updated

    async def resolve_ticket(
        self, ticket_id: int, resolution_note: str, resolved_by: int
    ) -> Ticket:
        """Resolve a ticket."""
        ticket = await self.get_ticket(ticket_id)
    ...
    async def rate_ticket(
        self, ticket_id: int, user_id: int, rating: int, feedback: str | None = None
    ) -> Ticket:
...
class FAQService:
    ...
    async def get_faq_by_slug(self, slug: str) -> FAQ | None:
        """Get FAQ by slug."""
...
__all__ = ["FAQService", "SupportService", "TicketService"]
