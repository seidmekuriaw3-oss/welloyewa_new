...
class TicketCategory(BaseModel, TimestampMixin):
    ...
    auto_assign_to: Mapped[int | None] = mapped_column(BigInteger, nullable=True)
...
class Ticket(BaseModel, TimestampMixin):
    ...
    category_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("ticket_categories.id"), nullable=True
    )
    ...
    ticket_metadata: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class FAQ(BaseModel, TimestampMixin):
    ...
    category_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("faq_categories.id"), nullable=True, index=True
    )

    question: Mapped[str] = mapped_column(String(500), nullable=False)
    question_am: Mapped[str | None] = mapped_column(String(500), nullable=True)
    answer: Mapped[str] = mapped_column(Text, nullable=False)
    answer_am: Mapped[str | None] = mapped_column(Text, nullable=True)
    slug: Mapped[str] = mapped_column(String(550), nullable=False, unique=True, index=True)

    # Metadata
    keywords: Mapped[list[str] | None] = mapped_column(JSON, nullable=True)  # Search keywords
...
class SupportArticle(BaseModel, TimestampMixin):
    ...
    content_am: Mapped[str | None] = mapped_column(Text, nullable=True)
