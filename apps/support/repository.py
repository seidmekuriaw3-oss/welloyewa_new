...
class TicketRepository(BaseRepository[Ticket]):
    ...
    async def get_overdue_tickets(self, sla_hours: int = 24) -> list[Ticket]:
        ...
        query = select(Ticket).where(
            Ticket.status.in_(["open", "in_progress"]), Ticket.created_at < cutoff
        )
        result = await self.db.execute(query)
...
class TicketMessageRepository(BaseRepository[TicketMessage]):
    ...
    async def get_last_message(self, ticket_id: int) -> TicketMessage | None:
...
class FAQRepository(BaseRepository[FAQ]):
    ...
    async def get_by_slug(self, slug: str) -> FAQ | None:
        """Get FAQ by slug."""
        query = select(FAQ).where(FAQ.slug == slug, FAQ.is_active)
        result = await self.db.execute(query)
        return result.scalar_one_or_none()
    ...
    async def get_by_category(
        ...
        query = select(FAQ).where(FAQ.category_id == category_id, FAQ.is_active)
    ...
    async def mark_helpful(self, faq_id: int) -> None:
        ...
        await self.db.execute(
            update(FAQ).where(FAQ.id == faq_id).values(helpful_count=FAQ.helpful_count + 1)
        )
        await self.db.flush()
    ...
    async def mark_not_helpful(self, faq_id: int) -> None:
        ...
        await self.db.execute(
            update(FAQ).where(FAQ.id == faq_id).values(not_helpful_count=FAQ.not_helpful_count + 1)
        )
        await self.db.flush()


__all__ = ["FAQRepository", "TicketMessageRepository", "TicketRepository"]
