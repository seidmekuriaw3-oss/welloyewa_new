...
class FAQEngine:
    ...
    async def get_all_categories_with_counts(self) -> list[dict[str, Any]]:
        ...
        result = await self.db.execute(
            select(FAQCategory).where(FAQCategory.is_active).order_by(FAQCategory.display_order)
        )
        ...
        for category in categories:
            count_result = await self.db.execute(
                select(func.count())
                .select_from(FAQ)
                .where(FAQ.category_id == category.id, FAQ.is_active)
            )
    ...
    async def get_faq_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_helpful_percentage": (
                (total_helpful / total_votes * 100) if total_votes > 0 else 0
            ),
        }
...
async def get_faq_by_slug(db: AsyncSession, slug: str) -> FAQ | None:
    """Get FAQ by slug."""
    engine = FAQEngine(db)
