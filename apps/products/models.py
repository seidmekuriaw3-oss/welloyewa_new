...
class Product(BaseModel, TimestampMixin, SoftDeleteMixin, MetadataMixin):
    ...
    vendor_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("vendors.id", ondelete="CASCADE"), nullable=False, index=True
    )

    # Basic info
    name: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    name_am: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ...
    category_type: Mapped[str | None] = mapped_column(
        SQLEnum(ProductCategory, name="product_category"), nullable=True, index=True
    )
    ...
    compare_price: Mapped[Decimal | None] = mapped_column(
        Numeric(10, 2), nullable=True
    )  # Original price for discount display
    cost_price: Mapped[Decimal | None] = mapped_column(Numeric(10, 2), nullable=True)  # Vendor cost
    ...
    sale_start_date: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    sale_end_date: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
...
class Review(BaseModel, TimestampMixin):
    ...
    comment_am: Mapped[str | None] = mapped_column(Text, nullable=True)
