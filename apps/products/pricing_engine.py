...
class PriceRule:
    ...
    action: Callable[[Decimal, dict[str, Any]], Decimal]
    priority: int = 0
    is_active: bool = True
...
class PricingEngine:
    ...
    def _init_default_rules(self) -> None:
        ...
        self.add_rule(
            PriceRule(
                ...
                action=lambda price, ctx: self._apply_sale_discount(price, ctx),
                priority=10,
            )
        )
    ...
    def calculate_price(
        ...
        for rule in self._rules:
            ...
            try:
                if rule.condition(context):
                    current_price = rule.action(current_price, context)
        ...
        discount_percentage = (
            float((discount_amount / original_price) * 100) if original_price > 0 else 0
        )
