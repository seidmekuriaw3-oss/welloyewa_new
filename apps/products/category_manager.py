...
class CategoryManager:
    ...
    async def get_category_product_count(
        self, category_id: int, include_subcategories: bool = True
    ) -> int:
        """
    ...
    async def update_category_product_counts(self, category_id: int) -> None:
        ...
        await self.db.execute(
            update(Category).where(Category.id == category_id).values(product_count=count)
        )
    ...
    async def move_category(self, category_id: int, new_parent_id: int | None) -> bool:
        """
    ...
    async def merge_categories(self, source_id: int, target_id: int) -> bool:
        ...
        await self.db.execute(
            update(Product).where(Product.category_id == source_id).values(category_id=target_id)
        )
...
async def get_category_tree(
    ...
    return await manager.get_category_tree(include_inactive)
