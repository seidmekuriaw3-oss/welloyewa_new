...
class ReviewManager:
    ...
    async def get_product_reviews(
        ...
        if approved_only:
            conditions.append(Review.is_approved)
    ...
    async def reject_review(self, review_id: int, reason: str | None = None) -> bool:
    ...
    async def update_product_rating(self, product_id: int) -> None:
        ...
        query = select(
            func.avg(Review.rating).label("avg_rating"), func.count().label("review_count")
        ...
        await self.db.flush()
        logger.debug(
            f"Updated rating for product {product_id}: {product.rating} ({product.reviews_count} reviews)"
        )
